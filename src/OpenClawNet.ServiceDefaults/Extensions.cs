...
public static class Extensions
{
    ...
    public static TBuilder AddServiceDefaults<TBuilder>(this TBuilder builder) where TBuilder : IHostApplicationBuilder
    {
        ...
        // Bind resilience tunables from the "Resilience" config section (defaults preserved when absent).
        var resilience = new ResilienceOptions();
        builder.Configuration.GetSection("Resilience").Bind(resilience);
        builder.Services.Configure<ResilienceOptions>(builder.Configuration.GetSection("Resilience"));

        builder.Services.ConfigureHttpClientDefaults(http =>
        {
            // Turn on resilience by default — AI/LLM inference calls can take 60-120s,
            // so the default 30s timeout is too short for this platform.
